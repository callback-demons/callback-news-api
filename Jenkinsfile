    ...
    stages {
        stage('Deploy') {
            steps {
                sh """
					#!/bin/bash
					ssh gerardo_marquez_carmona@api.callback-news.com -p 2222 'cd ~/callback-news-api && git pull && sudo pip3 install --ignore-installed -r app/requirements/base.txt -r app/requirements/prod.txt && sudo python3 app/manage.py migrate && sudo python3 app/manage.py collectstatic --noinput && sudo systemctl daemon-reload && sudo systemctl restart gunicorn
                    '
				"""
            }
        } 
